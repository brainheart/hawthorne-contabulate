#!/usr/bin/env python3
"""Build Contabulate JSON data files for Nathaniel Hawthorne's fiction corpus."""
...
def parse_standard_chapters(text):
    ...
    patterns = [
        ...
        (re.compile(r"^Chapter\s+([IVXLCDM]+|\d+)\s*$"), "Chapter"),
        (re.compile(r"^Chapter\s+([IVXLCDM]+|\d+)\s+(.+?)\s*$"), "Chapter"),
        # Standalone Roman numerals (Hawthorne's common chapter style)
        (re.compile(r"^([IVXLCDM]+)\.?\s*$"), "Chapter"),
        # Roman numeral + period + title on same line (e.g. "I. OLD MOODIE")
        (re.compile(r"^([IVXLCDM]+)\.\s+(.+?)\s*$"), "Chapter"),
        # CHAPTER I (no period, Marble Faun style)
        (re.compile(r"^CHAPTER\s+([IVXLCDM]+|\d+)\s*$"), "Chapter"),
    ]
...
def parse_story_collection(text, skip_preface=True):
    """Parse a story collection where each story starts with an ALL-CAPS title line."""
    lines = text.splitlines()
    # First pass: find the table of contents to get canonical titles
    toc_titles = []
    in_toc = False
    for i, raw in enumerate(lines[:120]):
        ...
        if in_toc:
            if not line:
                ...
                continue
            # Strip page numbers
            title = re.sub(r'\s+\d+\s*$', '', line).strip().rstrip('.')
            if title and len(title) > 2 and title == title.upper() and re.search(r'[A-Z]', title):
                toc_titles.append(title)

    # Second pass: find title lines in the body that match TOC entries
    starts = []
    body_start_idx = 0
    # Skip past any preface/intro matter
    ...
    if toc_titles:
        # Use TOC-based matching
        for i, raw in enumerate(lines[body_start_idx:], start=body_start_idx):
            line = raw.strip().rstrip('.')
            if line in toc_titles:
                starts.append((i, line))
    else:
        # Fallback: use all-caps lines that look like titles (> 4 chars, all upper, has letters)
        for i, raw in enumerate(lines[60:], start=60):  # skip header area
            line = raw.strip()
            if (line and len(line) > 4 and line == line.upper()
                    and re.search(r'[A-Z]{3}', line)
                    and not re.match(r'^(CONTENTS|PREFACE|INTRODUCTION|BY |NOTE|APPENDIX)', line)
                    and not re.fullmatch(r'[IVXLCDM\.\s]+', line)):
                starts.append((i, line))

    if not starts:
        return None

    # Deduplicate: if a title appears in TOC and then again as story header, keep only the body one
    seen_titles = {}
    deduped = []
    for idx, title in starts:
        if title in seen_titles:
            # Replace earlier (TOC) occurrence with this one
            deduped = [(i, t) for i, t in deduped if t != title]
        seen_titles[title] = idx
        deduped.append((idx, title))
    starts = deduped

    sections = []
    for si, (start_idx, title) in enumerate(starts):
        end_idx = starts[si + 1][0] if si + 1 < len(starts) else len(lines)
        body = '\n'.join(lines[start_idx + 1:end_idx]).strip()
        paragraphs = paragraphs_from_text(body)
        if not paragraphs:
            continue
        # Title-case the story name
        nice_title = title.title()
        sections.append({
            'number': len(sections) + 1,
            'label': f'Story {len(sections) + 1}',
            'title': nice_title,
            'paragraphs': paragraphs,
        })
...
STORY_COLLECTIONS = {'twice-told-tales', 'mosses', 'snow-image', 'wonder-book', 'tanglewood', 'great-stone-face'}

def parse_work_sections(work):
    work_id = work["id"]
    body = strip_gutenberg(read_file(work["file"]))

    if work_id in STORY_COLLECTIONS:
        sections = parse_story_collection(body)
        if sections:
            return sections
