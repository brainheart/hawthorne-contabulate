"""Sanity checks on the generated Hawthorne build output."""
...
class TestWorks(unittest.TestCase):
    ...
    def test_genre_counts(self):
        ...
        self.assertEqual(counts['Novel'], 6)
        self.assertEqual(counts['Stories'], 4)
        self.assertEqual(counts['Myth Retellings'], 2)
    ...
    def test_locations_follow_catalog_order(self):
        self.assertEqual(self.works[0]["location"], "01.TTT")
...
class TestParagraphs(unittest.TestCase):
    ...
    def test_paragraphs_are_present(self):
        self.assertEqual(len(self.paragraphs), 10452)

    def test_first_paragraph_shape(self):
        first = self.paragraphs[0]
        self.assertEqual(first["canonical_id"], "TTT.1.1")
        self.assertEqual(first["location"], "01.TTT.001.0001")
...
class TestTokens(unittest.TestCase):
    ...
    def test_token_index_size(self):
        self.assertGreater(len(self.tokens), 25000)

    def test_hawthorne_vocabulary_present(self):
        for word in ('scarlet', 'hester', 'puritan', 'gables'):
            self.assertIn(word, self.tokens)
        self.assertGreater(len(self.tokens["hester"]), 100)
